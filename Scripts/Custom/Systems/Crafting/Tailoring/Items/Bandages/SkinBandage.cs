...
{
    public class SkinBandage : Item, IDyable//, ICommodity
    {
        ...
        [Constructable]
        public SkinBandage(int amount)
            : base(0xE21)
        {
            Stackable = true;
            Amount = amount;
            Name = "bandagem de pele";
        }

        public SkinBandage(Serial serial)
            : base(serial)
        { }
      ...
      //  bool ICommodity.IsDeedable { get { return true; } }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from.InRange(GetWorldLocation(), Range))
            {
                ...
                from.SendMessage("Onde deseja usar essa bandagem?"); // Who will you use the bandages on?

                from.Target = new InternalTarget(this);
            }
            else
            {
                from.SendMessage("Você está longe demais"); // You are too far away to do that.
            }
        }

        public static void BandageTargetRequest(BandageTargetRequestEventArgs e)
        {
            BandageTargetRequest(e.Bandage as SkinBandage, e.Mobile, e.Target);
        }

        public static void BandageTargetRequest(SkinBandage bandage, Mobile from, Mobile target)
        {
        ...
        {
            private readonly SkinBandage m_SkinBandage;

            public InternalTarget(SkinBandage bandage)
                : base(SkinBandage.Range, false, TargetFlags.Beneficial)
            {
                m_SkinBandage = bandage;
            }

            protected override void OnTarget(Mobile from, object targeted)
            {
                if (m_SkinBandage.Deleted)
                {
                    return;
                }
    ...
    public class SkinBandageContext
    {
        ...
        public SkinBandageContext(Mobile healer, Mobile patient, TimeSpan delay)
            : this(healer, patient, delay, false)
        { }

        public SkinBandageContext(Mobile healer, Mobile patient, TimeSpan delay, bool enhanced)
        {
            m_Healer = healer;
            m_Patient = patient;
        ...
        private static readonly Dictionary<Mobile, SkinBandageContext> m_Table = new Dictionary<Mobile, SkinBandageContext>();

        public static SkinBandageContext GetContext(Mobile healer)
        {
            SkinBandageContext bc = null;
        ...
        {
            private SkinBandageContext m_Context;
            ...
            public InternalTimer(SkinBandageContext context, TimeSpan delay)
        ...
        public static SkinBandageContext BeginHeal(Mobile healer, Mobile patient)
        {
            return BeginHeal(healer, patient, false);
        }

        public static SkinBandageContext BeginHeal(Mobile healer, Mobile patient, bool enhanced)
        {
            ...
            else if (healer.CanBeBeneficial(patient, true, true))
            {
                ...
                SkinBandageContext context = GetContext(healer);
                ...
                context = new SkinBandageContext(healer, patient, delay, enhanced);
                ...
                {
                    healer.NetState.Send(new SkinBandageTimerPacket((int)delay.TotalSeconds));
                }
    ...
    public sealed class SkinBandageTimerPacket : Packet
    {
        public SkinBandageTimerPacket(int duration)
            : base(0xBF)
        {
            EnsureCapacity(15);
