...
namespace Server.Misc
{
    ...
    public class RegenRates
    {
        ...
        {
            Mobile.DefaultHitsRate = TimeSpan.FromSeconds(1.0);
            Mobile.DefaultStamRate = TimeSpan.FromSeconds(1.5);
            Mobile.DefaultManaRate = TimeSpan.FromSeconds(0.5);
        ...
        {
            double rate = 1.0 / (0.02 * (1 + HitPointRegen(from)));

            PlayerMobile pm = from as PlayerMobile;
            if (pm != null)
                rate = OSUDefQualDispatcher.ModifyHitsRegenRate(pm, rate);
        ...
        private static TimeSpan Mobile_StamRegenRate(Mobile from)
        {
            ...
            // ===== OSU Climate: multiplicador de regen (1.0 normal, menor = mais lento) =====
            double mult = 1.0;
            PlayerMobile pm = from as PlayerMobile;

            if (pm != null)
            {
                mult = Server.Custom.Systems.Climate.OSUClimatePenaltySystem.GetStamRegenMultiplier(pm);
            ...
            if (Core.SA)
            {
                // "rate" aqui é quantos segundos demora pra ganhar 1 ponto de stamina
                double rate = 1.0 / (0.25 + (bonus / 100));
                ...
                // mult < 1.0 => mais lento => tempo maior
                if (mult < 1.0)
                    rate = rate / mult;
            
                if (pm != null)
                    rate = OSUDefQualDispatcher.ModifyStamRegenRate(pm, rate);
            ...
            {
                // mesma lógica, só que a fórmula base é diferente
                double rate = 1.0 / (0.25 * (2 + bonus));
        ...
        private static TimeSpan Mobile_ManaRegenRate(Mobile from)
        {
            ...
            if (Core.ML)
            {
                ...
                rate = 1.0 / (0.05 + focusBonus + medBonus + itemBonus);
            }
            else if (Core.AOS)
            {
            ...
            if (pm != null)
            {
                rate = OSUDefQualDispatcher.ModifyManaRegenRate(pm, rate);

                if (pm.Meditating && OSUDefQualDispatcher.ShouldBreakMeditation(pm))
                {
                    pm.Meditating = false;
                    pm.SendMessage("Você perde a concentração e sai da meditação.");
                }
            }
