...
namespace Server.Spells
{
    ...
    public class SpellHelper
    {
        ...
        #region OSU
        // ===== OSU SKILL XP: GANHO AO ACERTAR FEITIÇO (SÓ MOBS HOSTIS) =====
        private static void OSUAwardSpellHitXP(Spell spell, Mobile from, Mobile target)
        {
            // Segurança básica
            if (spell == null || from == null || target == null)
                return;
            ...
            // Nunca dar XP batendo em player
            if (target is PlayerMobile)
                return;

            BaseCreature bc = target as BaseCreature;

            // Só dar XP se o alvo for uma criatura (mob)
            if (bc == null)
                return;

            // Nunca dar XP em vendors/NPCs
            if (bc is BaseVendor)
                return;

            // Nunca dar XP em pets/summons
            if (bc.Controlled || bc.Summoned)
                return;

            // Só dar XP em mobs hostis
            // (ServUO padrão: "AlwaysMurderer" é um bom indicador; Karma < 0 como fallback)
            if (!bc.AlwaysMurderer && bc.Karma >= 0)
                return;

            // Não dar XP em bicho sem fama (ou fama 0)
            if (bc.Fame <= 0)
                return;

            // Qual skill o feitiço usa (Magery/Necro/Chiv/Mysticism/etc)
            SkillName skillUsed = spell.CastSkill; // existe no Spell.cs :contentReference[oaicite:2]{index=2}

            // Dá XP (1 por acerto)
            Server.SkillXp.SkillXPSystem.AwardXP(pm, skillUsed, 1);
...
/* Tele To */		{ true,		true,		true,		true,		true,		true,			true,			true,		false,				true,				false,				false,		false, 		true,			true,						true,			true,				false,		false,      true,           false,          false,          false,          true },
        ...
        public static void Damage(Spell spell, TimeSpan delay, Mobile target, Mobile from, double damage)
        {
            ...
            if (delay == TimeSpan.Zero)
            {
                ...
                if (iDamage > 0)
                {
                    OSUAwardSpellHitXP(spell, from, target);
                }
                #endregion
        ...
        private class SpellDamageTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                if (m_Damage > 0)
                {
                    OSUAwardSpellHitXP(m_Spell, m_From, m_Target);
                }
                #endregion
        ...
        public class SpellDamageTimerAOS : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                if (damageGiven > 0 && target != null)
                {
                    OSUAwardSpellHitXP(m_Spell, m_From, target);
                }
                #endregion
