        ...
        */
        ...
        public virtual void OnHit(Mobile attacker, IDamageable damageable, double damageBonus)
		{
            ...
            */
            ...
            #region OSU
            // ===== OSU SKILL XP: GANHO AO ACERTAR GOLPE (SÓ MOBS HOSTIS) =====
            if (attacker is PlayerMobile pm)
            {
                ...
                // Só dar XP se o alvo for uma criatura
                if (bc == null)
                    return;
                ...
                // (ServUO padrão: "AlwaysMurderer" é o melhor indicador; Karma < 0 como fallback)
                if (!bc.AlwaysMurderer && bc.Karma >= 0)
                    return;

                // Só dar XP em mobs agressivos (que iniciam combate)
               // if (bc.FightMode == FightMode.Aggressor)
                //    return;

                if (bc.Fame <= 0)
                    return;

                SkillName skillUsed = this.Skill;
