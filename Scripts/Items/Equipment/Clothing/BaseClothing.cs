...
namespace Server.Items
{
    ...
    public abstract class BaseClothing : Item, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability, IResource, ISetItem, IVvVItem, IOwnerRestricted, IArtifact, ICombatEquipment, IEngravable, IQuality
    {
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            switch (OSUFabricType)
            {
                case 1: tecido = "Lã"; break;
                case 2: tecido = "Pele"; break;
                case 3: tecido = "Linho"; break;
                case 4: tecido = "Seda"; break;
                case 5: tecido = "Algodão"; break;
                default: tecido = "Algodão"; break;
            }

            list.Add($"Tecido: {tecido}");
        ...
        public virtual bool Scissor(Mobile from, Scissors scissors)
        {
            ...
            if (item != null && item.Resources.Count == 1 && item.Resources.GetAt(0).Amount >= 2)
            {
                try
                {
                    ...
                    // OSU: ao cortar roupa de pano, devolver tecido conforme OSUFabricType
                    if (resourceType == typeof(Cloth) || typeof(Cloth).IsAssignableFrom(resourceType))
                    {
                        switch (OSUFabricType)
                        {
                            case 4: // Seda
                                resourceType = typeof(SilkCloth);
                                break;

                            case 1: // Lã
                                resourceType = typeof(WoolCloth);
                                break;

                            case 3: // Linho
                                resourceType = typeof(LinenCloth);
                                break;

                            case 2: // Pele 
                                resourceType = typeof(SkinCloth);
                                break;

                            case 5: // Algodao 
                                resourceType = typeof(CottonCloth);
                                break;

                            default: // padrao desce algodao
                                resourceType = typeof(CottonCloth);
                                break;
                        }
                    }
        ...
        public virtual int OnCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, ITool tool, CraftItem craftItem, int resHue)
        {
            ...
            try
            {
                // 1) Começa com o typeRes padrão
                Type resType = typeRes;

                // 2) Se o craftSystem existir, tenta pegar o material que o jogador escolheu no menu (SubRes2)
                if (craftSystem != null && from != null)
                {
                    var ctx = craftSystem.GetContext(from);

                    // LastResourceIndex2 = material de tecido selecionado no botão (SubRes2)
                    if (ctx != null && ctx.LastResourceIndex2 >= 0 && craftSystem.CraftSubRes2 != null && craftSystem.CraftSubRes2.Init)
                    {
                        resType = craftSystem.CraftSubRes2.GetAt(ctx.LastResourceIndex2).ItemType;
                    }
                }

                // 3) Fallback: se ainda for null, usa o primeiro recurso do craftItem
                if (resType == null && craftItem != null && craftItem.Resources != null && craftItem.Resources.Count > 0)
                    resType = craftItem.Resources.GetAt(0).ItemType;

                // 4) Agora sim seta o OSUFabricType certo
                if (resType == typeof(LinenCloth))
                    this.OSUFabricType = 3; // Linho
                else if (resType == typeof(WoolCloth))
                    this.OSUFabricType = 1; // Lã
                else if (resType == typeof(SilkCloth))
                    this.OSUFabricType = 4; // Seda
                else if (resType == typeof(SkinCloth))
                    this.OSUFabricType = 2; // Pele
                else if (resType == typeof(CottonCloth))
                    this.OSUFabricType = 5; // algodão
                else
                    this.OSUFabricType = 0; // Algodão/padrão
            }
            catch
            {
                this.OSUFabricType = 0;
            }
        ...
        // ===== OSU Fabrics =====
        private int _OSUFabricType; // 0 = Cotton

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUFabricType
        {
            get { return _OSUFabricType; }
            set { _OSUFabricType = value; InvalidateProperties(); }
        }
