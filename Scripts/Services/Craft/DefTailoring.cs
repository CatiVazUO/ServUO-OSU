...
namespace Server.Engines.Craft
{
    ...
    public class DefTailoring : CraftSystem
    {
        ...
        public override void InitCraftList()
        {
            ...
            // ===== OSU: Tecido como SubRes2 (separado do couro) =====
            SetSubRes2(typeof(Cloth), "Tecido");

            AddSubRes2(typeof(CottonCloth), "Algodão", 0.0, "você não sabe trabalhar com esse material");
            AddSubRes2(typeof(WoolCloth), "Lã", 55.0, "você não sabe trabalhar com esse material");
            AddSubRes2(typeof(LinenCloth), "Linho", 55.0, "você não sabe trabalhar com esse material");
            AddSubRes2(typeof(SilkCloth), "Seda", 90.0, "você não sabe trabalhar com esse material");
            AddSubRes2(typeof(SkinCloth), "Pele", 90.0, "você não sabe trabalhar com esse material");
            ...
            // ====== OSU: fazer TODOS os crafts que usam Cloth obedecerem o material do SubRes2 ======
            for (int i = 0; i < CraftItems.Count; i++)
            {
                CraftItem ci = CraftItems.GetAt(i);

                if (ci == null || ci.Resources == null || ci.Resources.Count == 0)
                    continue;

                // Se o craft usa qualquer tipo derivado de Cloth (inclui Cloth e CottonCloth), ele deve obedecer o SubRes2
                Type firstRes = ci.Resources.GetAt(0).ItemType;

                if (firstRes == typeof(Cloth) || typeof(Cloth).IsAssignableFrom(firstRes))
                {
                    ci.UseSubRes2 = true;
                }
                if (ci.Resources.GetAt(0).ItemType == typeof(Cloth))
                {
                    ci.UseSubRes2 = true;
                }
            }
