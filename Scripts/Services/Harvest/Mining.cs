    ...
    public class Mining : HarvestSystem
    {
        ...
        private Mining()
        {
            ...
            oreAndStone.Tiles = m_MountainAndCaveTiles
                .Concat(m_OSUStoneTiles)
                .Concat(m_OSUStoneBlockTiles)
                .Concat(m_OSUGraniteTiles)
                .Concat(m_OSUGraniteBlockTiles)
                .Concat(m_OSUMarbleTiles)
                .Concat(m_OSUMarbleBlockTiles)
                .Concat(m_OSUSoapstoneTiles)
                .Concat(m_OSUSoapstoneBlockTiles)
                .Concat(m_OSUClayTiles)
                .Concat(m_OSUClayPileTiles)
                .Concat(m_OSUKaolinTiles)
                .Concat(m_OSUQuartzTiles)
                .Concat(m_OSUTinTiles)
                .Concat(m_OSUCopperTiles)
                .ToArray();
        ...
        private static bool ContainsTile(int[] list, int tileID)
        {
            if (list == null)
                return false;
        ...
        private static int GetCustomMiningTileID(Map map, Point3D loc)
        {
            if (map == null || map == Map.Internal)
                return -1;

            LandTile land = map.Tiles.GetLandTile(loc.X, loc.Y);
            int landID = land.ID & 0x3FFF;

            if (ContainsTile(m_OSUStoneTiles, landID) ||
                ContainsTile(m_OSUStoneBlockTiles, landID) ||
                ContainsTile(m_OSUGraniteTiles, landID) ||
                ContainsTile(m_OSUGraniteBlockTiles, landID) ||
                ContainsTile(m_OSUMarbleTiles, landID) ||
                ContainsTile(m_OSUMarbleBlockTiles, landID) ||
                ContainsTile(m_OSUSoapstoneTiles, landID) ||
                ContainsTile(m_OSUSoapstoneBlockTiles, landID) ||
                ContainsTile(m_OSUClayTiles, landID) ||
                ContainsTile(m_OSUClayPileTiles, landID) ||
                ContainsTile(m_OSUKaolinTiles, landID) ||
                ContainsTile(m_OSUQuartzTiles, landID) ||
                ContainsTile(m_OSUTinTiles, landID) ||
                ContainsTile(m_OSUCopperTiles, landID))
                return landID;

            StaticTile[] statics = map.Tiles.GetStaticTiles(loc.X, loc.Y, true);

            for (int i = 0; i < statics.Length; i++)
            {
                int staticID = statics[i].ID & 0x3FFF;

                if (ContainsTile(m_OSUStoneTiles, staticID) ||
                    ContainsTile(m_OSUStoneBlockTiles, staticID) ||
                    ContainsTile(m_OSUGraniteTiles, staticID) ||
                    ContainsTile(m_OSUGraniteBlockTiles, staticID) ||
                    ContainsTile(m_OSUMarbleTiles, staticID) ||
                    ContainsTile(m_OSUMarbleBlockTiles, staticID) ||
                    ContainsTile(m_OSUSoapstoneTiles, staticID) ||
                    ContainsTile(m_OSUSoapstoneBlockTiles, staticID) ||
                    ContainsTile(m_OSUClayTiles, staticID) ||
                    ContainsTile(m_OSUClayPileTiles, staticID) ||
                    ContainsTile(m_OSUKaolinTiles, staticID) ||
                    ContainsTile(m_OSUQuartzTiles, staticID) ||
                    ContainsTile(m_OSUTinTiles, staticID) ||
                    ContainsTile(m_OSUCopperTiles, staticID))
                    return staticID;
            }
        ...
        private static int GetHarvestTileID(object toHarvest)
        {
            if (toHarvest is StaticTarget)
                return ((StaticTarget)toHarvest).ItemID & 0x3FFF;

            if (toHarvest is LandTarget)
                return ((LandTarget)toHarvest).TileID & 0x3FFF;
        ...
        public override Type GetResourceType(Mobile from, Item tool, HarvestDefinition def, Map map, Point3D loc, HarvestResource resource)
        {
            if (def == OreAndStone)
            {
                int osuTileID = GetCustomMiningTileID(map, loc);

                if (ContainsTile(m_OSUStoneBlockTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.05) ? typeof(StoneBlockLarge) : typeof(StoneBlockSmall);

                if (ContainsTile(m_OSUStoneTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.10) ? typeof(StoneBlockSmall) : typeof(StoneChunk);

                if (ContainsTile(m_OSUGraniteBlockTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.05) ? typeof(GraniteBlockLarge) : typeof(GraniteBlockSmall);

                if (ContainsTile(m_OSUGraniteTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.10) ? typeof(GraniteBlockSmall) : typeof(GraniteChunk);

                if (ContainsTile(m_OSUMarbleBlockTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.05) ? typeof(MarbleBlockLarge) : typeof(MarbleBlockSmall);

                if (ContainsTile(m_OSUMarbleTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.10) ? typeof(MarbleBlockSmall) : typeof(MarbleChunk);

                if (ContainsTile(m_OSUSoapstoneBlockTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.05) ? typeof(SoapstoneBlockLarge) : typeof(SoapstoneBlockSmall);

                if (ContainsTile(m_OSUSoapstoneTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.10) ? typeof(SoapstoneBlockSmall) : typeof(SoapstoneChunk);

                if (ContainsTile(m_OSUClayPileTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.05) ? typeof(ClayPileLarge) : typeof(ClayPileSmall);

                if (ContainsTile(m_OSUClayTiles, osuTileID))
                    return (Utility.RandomDouble() < 0.10) ? typeof(ClayPileSmall) : typeof(ClayChunk);

                if (ContainsTile(m_OSUKaolinTiles, osuTileID))
                    return typeof(KaolinChunk);

                if (ContainsTile(m_OSUQuartzTiles, osuTileID))
                    return typeof(QuartzChunk);

                if (ContainsTile(m_OSUTinTiles, osuTileID))
                    return typeof(TinNugget);

                if (ContainsTile(m_OSUCopperTiles, osuTileID))
                    return typeof(CopperNugget);
        ...
        private static readonly int[] m_OSUStoneTiles = new int[]
 {
    0x5000, 0x5001
 };

        private static readonly int[] m_OSUStoneBlockTiles = new int[]
        {
    0x5002
        };

        private static readonly int[] m_OSUGraniteTiles = new int[]
        {
    0x5003, 0x5004
        };

        private static readonly int[] m_OSUGraniteBlockTiles = new int[]
        {
    0x5005
        };

        private static readonly int[] m_OSUMarbleTiles = new int[]
        {
    0x5006, 0x5007
        };

        private static readonly int[] m_OSUMarbleBlockTiles = new int[]
        {
    0x5008
        };

        private static readonly int[] m_OSUSoapstoneTiles = new int[]
        {
    0x5009, 0x500A
        };

        private static readonly int[] m_OSUSoapstoneBlockTiles = new int[]
        {
    0x500B
        };

        private static readonly int[] m_OSUClayTiles = new int[]
        {
    0x500C, 0x500D
        };

        private static readonly int[] m_OSUClayPileTiles = new int[]
        {
    0x500E
        };

        private static readonly int[] m_OSUKaolinTiles = new int[]
        {
    0x500F
        };

        private static readonly int[] m_OSUQuartzTiles = new int[]
        {
    0x5010
        };

        private static readonly int[] m_OSUTinTiles = new int[]
        {
    0x5011
        };

        private static readonly int[] m_OSUCopperTiles = new int[]
        {
    0x5012
        };
