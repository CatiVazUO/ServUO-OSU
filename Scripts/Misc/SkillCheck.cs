...
namespace Server.Misc
{
	public class SkillCheck
	{
        ...
        private static double GetGainChance(Mobile from, Skill skill, double gains, double chance)
        {
            ...
            if (pm != null)
                gc = OSUDefQualDispatcher.ModifySkillGainScalar(pm, gc);
            ...
            //  gc *= 0.25;   // AQUI MUDA O UP GERAL DE SKILLS 0.25 faz com q demora 75 % a mais pra upar a skill
        ...
        public static void Gain(Mobile from, Skill skill, int toGain)
		{
					...
					if (minsPerGain > 0)
					{
						if (Siege.CheckSkillGain((PlayerMobile)from, minsPerGain, skill))
						{
							...
							if (skills.Total + toGain <= skills.Cap)
							{
                                ...
                                {
                                    SkillName sn = skill.SkillName; // ou skillName dependendo do arquivo
                                    Server.SkillXp.SkillXPSystem.OSUSkillGroup g = Server.SkillXp.SkillXPSystem.GetSkillGroup(sn);

                                    if (g == Server.SkillXp.SkillXPSystem.OSUSkillGroup.Combat ||
                                        g == Server.SkillXp.SkillXPSystem.OSUSkillGroup.Profession)
                                    {
                                        double total = Server.SkillXp.SkillXPSystem.GetTotalSkillValueForGroup(pm, g);
                                        double cap = Server.SkillXp.SkillXPSystem.GetGroupCap(pm, g);
			...
			//		toGain = Utility.Random(4) + 1;
				...
				if (!from.Player || (skills.Total + toGain <= skills.Cap))
				{
                    ...
                    if (from is PlayerMobile)
                    toGain = 1; // SEMPRE +0.1 para jogadoresoGain = 1;
