...
namespace Server.Custom.Reinos
{
    public static class ReinoTrialsSystem
    {
        ...
        public static string StartSession(PlayerMobile judge, int cityId, PlayerMobile accused)
        {
            ...
            if (!IsInsideTribunal(cityId, accused.Location, accused.Map))
                return "O jogador precisa estar dentro do tribunal.";
        ...
        public static bool HandleCourtCrime(PlayerMobile pm, int cityId, string crimeLabel, bool contempt)
        {
            ...
            OSUJusticeOfficer officer = FindJusticeOfficer(cityId);
            bool lootStored = ReinoMilitarySystem.ConfiscateLivingPrisonerItems(pm, cityId);
            bool prisoned = false;

            if (officer != null)
                prisoned = ReinoMilitarySystem.TrySendToPrison(pm, cityId, officer, ReinoMilitaryLaw.Fighting);
            else
                prisoned = ReinoMilitarySystem.TrySendToPrison(pm, cityId, null, ReinoMilitaryLaw.Fighting);

            if (!prisoned)
            {
                ReinoConstructionRuntimeInfo prison = FindPrimaryPrisonRuntime(cityId);
                if (prison != null && prison.Lot != null && prison.Lot.Map == pm.Map)
                {
                    Point3D p = prison.Lot.GetCenter(prison.Lot.NorthWest.Z);
                    pm.MoveToWorld(p, pm.Map);
                    prisoned = true;
                }
            }
