...
namespace Server.Mobiles
{
    ...
    public class BaseCreature : Mobile, IHonorTarget, IEngravable
    {
        ...
        #region All OSUs Stuff

        #region Seters and Getters
        // Statues
        public virtual bool StatueCanBeLiveModel { get { return false; } }
        public virtual int StatueMaterialAmount { get { return 0; } }
        public virtual StatuePlatformSize StatuePlatformSize
        {
            get { return StatuePlatformSize.None; }
        }
        ...
        public int OSULives { get; set; } = 0;
        public int OSUReviveSeconds { get; set; } = 0;
        ...
        private bool OSUHasLives => (OSULivesMax > 0);


        #endregion

        #region OSU Lives (Mobs) - Corpse lootável + revive (SEM mexer em Serialize)

        // ===== Tabela estática de revives pendentes por corpse =====
        private sealed class OSUReviveInfo
        {
            public Type Type;
            public Map Map;
        ...
        private static readonly Dictionary<Serial, OSUReviveInfo> _OSUReviveInfoByCorpse = new Dictionary<Serial, OSUReviveInfo>();
        private static readonly Dictionary<Serial, Timer> _OSUReviveTimerByCorpse = new Dictionary<Serial, Timer>();

        protected virtual void OnOSULifeLost(int livesRemaining) { }
        protected virtual void OnOSURevived(int livesRemaining) { }

        // ===== Agenda revive a partir do corpse =====
        private static void OSUScheduleReviveFromCorpse(Corpse corpse, BaseCreature dead, int livesRemaining, int livesMax, int reviveSeconds)
        {
            if (corpse == null || corpse.Deleted || dead == null)
                return;

            var info = new OSUReviveInfo
            {
                Type = dead.GetType(),
                Map = corpse.Map,
                ...
                LivesMax = livesMax,
                ReviveSeconds = reviveSeconds < 1 ? 1 : reviveSeconds
            };

            Timer old;
            if (_OSUReviveTimerByCorpse.TryGetValue(corpse.Serial, out old) && old != null)
                old.Stop();

            _OSUReviveInfoByCorpse[corpse.Serial] = info;

            Timer t = Timer.DelayCall(TimeSpan.FromSeconds(info.ReviveSeconds), () =>
            {
                OSUReviveInfo i2;
                if (_OSUReviveInfoByCorpse.TryGetValue(corpse.Serial, out i2) && i2 != null)
                {
                    _OSUReviveInfoByCorpse.Remove(corpse.Serial);

                    Timer tt;
                    if (_OSUReviveTimerByCorpse.TryGetValue(corpse.Serial, out tt) && tt != null)
                        tt.Stop();

                    _OSUReviveTimerByCorpse.Remove(corpse.Serial);

                    OSUDoReviveNow(corpse.Serial, i2);
                }
            });

            _OSUReviveTimerByCorpse[corpse.Serial] = t;
        }

        // ===== Revive imediato (usado pelo Carve) =====
        public static bool OSUTryForceReviveFromCorpse(Corpse corpse, Mobile from)
        {
            if (corpse == null || corpse.Deleted)
                return false;

            OSUReviveInfo info;
            if (!_OSUReviveInfoByCorpse.TryGetValue(corpse.Serial, out info) || info == null)
                return false;

            // cancela timer pendente
            Timer t;
            if (_OSUReviveTimerByCorpse.TryGetValue(corpse.Serial, out t) && t != null)
                t.Stop();

            _OSUReviveTimerByCorpse.Remove(corpse.Serial);
            _OSUReviveInfoByCorpse.Remove(corpse.Serial);

            from?.SendMessage(0x22, "Você tenta abrir o corpo, mas o sente ainda pulsando.");

            OSUDoReviveNow(corpse.Serial, info);
            return true;
        }

        private static void OSUDoReviveNow(Serial corpseSerial, OSUReviveInfo info)
        {
            Corpse corpse = World.FindItem(corpseSerial) as Corpse;
            if (info == null || info.Map == null)
                return;

            BaseCreature bc = null;
            try { bc = Activator.CreateInstance(info.Type) as BaseCreature; }
            catch { bc = null; }

            if (bc == null)
                return;

            // impede loot “novo” no spawn
            bc.OSUSuppressSpawnLoot = true;

            // aplica vidas
            bc.OSULivesMax = info.LivesMax;
            bc.OSULives = info.LivesRemaining;
            bc.OSUReviveSeconds = info.ReviveSeconds;

            // remove qualquer item criado no construtor (bones, etc.)
            foreach (var it in bc.Items.ToArray())
                it?.Delete();

            // garante backpack vazia
            Backpack pack = new Backpack { Movable = false };
            bc.AddItem(pack);

            // spawn no local do corpse
            bc.MoveToWorld(info.Location, info.Map);

            // transfere o que SOBROU no corpse para o mob revive (equip + backpack)
            if (corpse != null && !corpse.Deleted)
            {
                // tenta equipar primeiro
                Item[] items = corpse.Items.ToArray();
                for (int i = 0; i < items.Length; i++)
                {
                    Item it = items[i];
                    if (it == null || it.Deleted || it.Parent != corpse)
                        continue;

                    if (it.Layer != Layer.Invalid && it.Layer != Layer.Backpack && it.Layer != Layer.Bank)
                    {
                        corpse.RemoveItem(it);
                        if (bc.FindItemOnLayer(it.Layer) == null)
                        {
                            bc.EquipItem(it);
                            if (it.Parent != bc)
                                pack.DropItem(it);
                ...
                // resto vai pra backpack
                items = corpse.Items.ToArray();
                for (int i = 0; i < items.Length; i++)
                {
                    Item it = items[i];
                    if (it == null || it.Deleted || it.Parent != corpse)
            ...
            bc.OSUSuppressSpawnLoot = false;
            bc.OnOSURevived(info.LivesRemaining);
        }
        ...
        */
        ...
        public virtual void GenerateLoot(bool spawning)
        {
            ...
            // OSU Lives: se este mob foi recriado por revive, não gerar loot de spawn
            if (spawning && OSUSuppressSpawnLoot)
                return;
        ...
        public override bool OnBeforeDeath()
        {
            ...
            // ===== OSU Lives (mobs): intercepta morte para "knockout" =====
            if (OSUHasLives)
            {
                // Se ainda tem mais de 1 vida, esta morte é intermediária (knockout)
                if (OSULives > 1)
                {
                    OSULives = Math.Max(0, OSULives - 1);

                    // Marca que o OnDeath deve criar corpse vazio + agendar revive
                    _OSUKnockoutPending = true;

                    // IMPORTANTÍSSIMO: impede o loot normal desta morte
                    m_HasGeneratedLoot = true;

                    // Deixa o corpse vazio (remove qualquer coisa que esteja no backpack)
                    if (Backpack != null)
                    {
                        ColUtility.SafeDelete(Backpack.Items);
                    }

                    OnOSULifeLost(OSULives);

                    // Deixa o fluxo normal criar o corpse (mas sem loot)
                    return base.OnBeforeDeath();
                }

                // Se chegou aqui e era 1, é a morte final
                if (OSULives == 1)
                    OSULives = 0;
            }
        ...
        public virtual void OnKilledBy(Mobile mob)
        {
            ...
            {
                int bonusGold = OSUDefQualDispatcher.ModifyFinalBlowGold(pm, 0);

                if (bonusGold > 0)
                {
                    pm.AddToBackpack(new Gold(bonusGold));
                    pm.SendMessage("Você encontrou algumas moedas extras no corpo da criatura.");
                }
            }
        ...
        public override void OnDeath(Container c)
        {
            ...
            // ===== OSU Lives (mobs): morte intermediária -> corpse lootável + revive =====
            if (OSUHasLives && _OSUKnockoutPending)
            {
                _OSUKnockoutPending = false;

                // Cria corpse/execução padrão (mas sem loot extra, já bloqueado no OnBeforeDeath via m_HasGeneratedLoot)
                base.OnDeath(c);

                if (c is Corpse corpse && corpse != null && !corpse.Deleted)
                {
                    corpse.OSUKnockoutCorpse = true; // <- adiciona isso
                    OSUScheduleReviveFromCorpse(corpse, this, OSULives, OSULivesMax, OSUReviveSeconds);
                }
                return;
            }
