...
namespace Server.Mobiles
{
    ...
    public partial class PlayerMobile : Mobile, IHonorTarget
    {
        ...
        // Próximo tick de recuperação de vida (UTC). DateTime.MinValue = sem timer (já está cheio)
        private DateTime _OSUNextLifeRegen;

        // Corpse usado no último desmaio (para restaurar itens remanescentes)
        private int _OSUKnockoutCorpseSerial;

        // Flags internos de fluxo de morte
        ...
        // Config: área onde o fantasma fica durante o desmaio (ajuste para o seu mapa)
        public static readonly Point3D OSUKnockoutLocation = new Point3D(1000, 1000, 0);
        public static readonly Map OSUKnockoutMap = Map.Felucca;

        // Config: área do "céu" (permadeath)
        public static readonly Point3D OSUHeavenLocation = new Point3D(1100, 1100, 0);
        public static readonly Map OSUHeavenMap = Map.Felucca;

        //Flags do sistema de xp
        public int OSUFeatSpentXPCombat;   // gasto em feats de combate
        public int OSUFeatSpentXPProf;     // gasto em feats de profissão
        public bool OSUFeatCapsInverted;   // NPC futuro pode inverter caps
        public bool OSUInitialSkillsLocked;
        private HashSet<SkillName> _OSUUnlockedSkills; // skills desbloqueadas
        public bool OSUSkillGumpOpened;
        ...
        public SculptorRequirement[] StatueExtraRequirements
        {
            get
            {
        ...
        public StatuePlatformSize StatuePlatformSize
        {
            get { return StatuePlatformSize.Medium; }
        }

        public int StatueSuccessChance
        {
            get { return 80; }
        }
        public StatuePoseDefinition[] StatueAllowedPoses
        {
            get
            {
                return new StatuePoseDefinition[]
                {
            new StatuePoseDefinition("Idle", 0, 1, true),
            new StatuePoseDefinition("Saudação", 33, 3, true),
            new StatuePoseDefinition("Combate", 31, 5, true),
            new StatuePoseDefinition("Braços erguidos", 17, 5, true)
                };
            }
        }
        ...
        public int OSUHunger { get; set; } = 100;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUThirst { get; set; } = 100;

        // penalidades acumuladas enquanto <10 (valores negativos)
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUNeedsStrPenalty { get; set; } = 0;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUNeedsDexPenalty { get; set; } = 0;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUNeedsIntPenalty { get; set; } = 0;


        // ===== OSU: Defeitos & Qualidades escolhidos (flags permanentes) =====
        [CommandProperty(AccessLevel.GameMaster)]
        public List<string> OSUDefQualFlags { get; set; } = new List<string>();

        public bool HasOSUDefQual(string id)
        {
            if (OSUDefQualFlags == null || String.IsNullOrWhiteSpace(id))
                return false;

            for (int i = 0; i < OSUDefQualFlags.Count; i++)
                if (String.Equals(OSUDefQualFlags[i], id, StringComparison.OrdinalIgnoreCase))
                    return true;

            return false;
        }

        // ===== OSU LevelUp Stats draft (apenas para o gump funcionar entre cliques) =====
        public bool OSULvlDraftActive { get; set; }

        public int OSULvlBaseStr { get; set; }
        public int OSULvlBaseDex { get; set; }
        public int OSULvlBaseInt { get; set; }
        public int OSULvlBaseHP { get; set; }
        public int OSULvlBaseStam { get; set; }
        public int OSULvlBaseMana { get; set; }

        public int OSULvlDraftStr { get; set; }
        public int OSULvlDraftDex { get; set; }
        public int OSULvlDraftInt { get; set; }
        public int OSULvlDraftHP { get; set; }
        public int OSULvlDraftStam { get; set; }
        public int OSULvlDraftMana { get; set; }


        // ===== OSU LevelUp Stats =====
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUPendingStatPoints { get; set; } // pontos pendentes de distribuir (20 por nível)

        // distribuição virtual (valores “propostos” no gump)
        public int OSULvl_Str { get; set; }
        public int OSULvl_Dex { get; set; }
        public int OSULvl_Int { get; set; }
        public int OSULvl_HP { get; set; }
        public int OSULvl_Stam { get; set; }
        public int OSULvl_Mana { get; set; }

        public bool OSULvl_InProgress { get; set; } // se já inicializou os valores pro gump



        // ===== OSU: atributos base (os escolhidos na criação) =====
        // Esses são os "valores reais" que o player escolheu


        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUBaseHP { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUBaseStam { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUBaseMana { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUBaseDex { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUBaseStr { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUBaseInt { get; set; }

        // ===== OSU: caps máximos que Def/Qual alteram (ex.: Frágil) =====
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUHpCapMax { get; set; } = 115;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUStamCapMax { get; set; } = 115;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUManaCapMax { get; set; } = 115;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUStrCapMax { get; set; } = 115;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUDexCapMax { get; set; } = 115;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUIntCapMax { get; set; } = 115;

        // ===== OSU Creation - persistente =====
        [CommandProperty(AccessLevel.GameMaster)]
        public bool OSUCreationCompleted { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool OSUIsPvpChar { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public string OSUCultureId { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public string OSUCitizenCityId { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public string OSUReligionId { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUAvatarId { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSURpWeightKg { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSURpHeightCm { get; set; }


        // idade
        private int _OSURpAgeBase;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSURpAgeBase
        {
            get => _OSURpAgeBase;
            set => _OSURpAgeBase = value;
        }

        // Mostra idade dinâmica no [props]
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSURpAge
        {
            get => Custom.Systems.WorldTime.OSUAgeHelper.GetCurrentAge(this);
            set => _OSURpAgeBase = value; // se GM mudar OSURpAge, muda a base
        }

        // Data do mundo quando o char terminou a criação
        [CommandProperty(AccessLevel.GameMaster)]
        public DateTime OSURpBirthWorldTime { get; set; }


        [CommandProperty(AccessLevel.GameMaster)]
        public string OSURpTraitsPublic { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public string OSURpHistoryStaff { get; set; }

        // ===== OSU Feat Caps - custom por personagem (def/qual) =====
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUFeatTotalCapCustom { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUFeatCombatCapCustom { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUFeatProfessionCapCustom { get; set; }

        // ===== OSU Skill Caps por personagem (Combat x Craft/Profession) =====
        [CommandProperty(AccessLevel.GameMaster)]
        public double OSUCombatSkillCap { get; set; } = 400.0;

        [CommandProperty(AccessLevel.GameMaster)]
        public double OSUCraftSkillCap { get; set; } = 300.0;

        public OSUCreationContext OSUCreation { get; set; }

        public bool OSUPermaDead { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSULivesMax { get; set; } = 3;

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSULives { get; set; } = 3;

        // Intervalo para recuperar 1 vida (em segundos). Default: 3600 = 1h
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSULifeRegenSeconds { get; set; } = 3600;

        // Tempo de "desmaio" (em segundos). Default: 60 = 1min
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUKnockoutSeconds { get; set; } = 60;

        [CommandProperty(AccessLevel.GameMaster)]
        public double OSULifeRegenScalar { get; set; } = 1.0;

        [CommandProperty(AccessLevel.GameMaster)]
        public double OSUKnockoutScalar { get; set; } = 1.0;

        // Multiplicadores (traits/buffs)
        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUGeneralXP { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSULevel { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUNextLevelXP { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUAbilityPicks { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int OSUFeatSpentXP { get; set; }

        public int OSU_TacticsCarry { get; set; }
        public int OSU_EvalCarry { get; set; }
        ...
        public bool IsCitizenOf(string cityId)
        {
            if (String.IsNullOrWhiteSpace(cityId))
                return true; // banco sem cidade definida -> libera

            return String.Equals(OSUCitizenCityId, cityId, StringComparison.OrdinalIgnoreCase);
        }
        ...
        public bool TrySetReligion(string religionId, out string reason)
        {
            reason = null;

            if (String.IsNullOrWhiteSpace(religionId))
            {
                reason = "Religião inválida.";
                return false;
            }

            OSUReligionId = religionId.Trim();
            return true;
        }

        #endregion

        #region OSU Feats\Abilities

        private List<int> _OSUAbilities;

        public List<int> OSUAbilities
        {
            get
            {
                if (_OSUAbilities == null)
                    _OSUAbilities = new List<int>();
                return _OSUAbilities;
            }
        }

        public bool HasOSUAbility(int id)
        {
            return _OSUAbilities != null && _OSUAbilities.Contains(id);
        }


        private List<int> _OSUFeats;

        public List<int> OSUFeats
        {
            get
            {
                if (_OSUFeats == null)
                    _OSUFeats = new List<int>();

                return _OSUFeats;
            }
        }

        public bool HasOSUFeat(int featId)
        {
            return _OSUFeats != null && _OSUFeats.Contains(featId);
        }

        #endregion

        #region OSU Char Creation

        public void ResetForOSUCreation()
        {
            // ===== 1) Nome padrão durante a criação =====
            Name = "Sonhador";

            // ===== 2) Aparência base "neutra" (você vai escolher depois no gump) =====
            // Mantém humano padrão (sem cabelo/barba/roupas). Depois a gente faz a página visual.
            Hue = 0;

            // Remove cabelo/barba se existir
            Item hair = FindItemOnLayer(Layer.Hair);
            if (hair != null) hair.Delete();
            ...
            // ===== 3) Stats base zerados (o gump vai decidir depois) =====
            // Aqui eu deixei tudo em 10 pra não quebrar nada (0 pode dar bug em alguns sistemas).
            RawStr = 10;
            RawDex = 10;
            RawInt = 10;

            // Atualiza hits/stam/mana para o "máximo" atual
            Hits = HitsMax;
            Stam = StamMax;
            Mana = ManaMax;

            // ===== 4) Skills: tudo 0 e tudo LOCKED =====
            ...
            // ===== 5) Limpa escolhas OSU relacionadas a feats/abilities/unlocks (pra não "vazar" nada) =====
            // (esses campos existem no seu PlayerMobile)
            try
            {
                // travas e escolhas
                OSUInitialSkillsLocked = true;

                // limpa feats/abilities (listas/dicionários)
                // Se no seu PlayerMobile esses campos forem exatamente esses nomes, isso compila.
                // Se algum nome estiver diferente, você me fala qual erro apareceu que eu ajusto.
                if (OSUFeats != null)
                    OSUFeats.Clear();

                // Se você tiver lista de abilities guardada em lista privada, a gente limpa no ponto certo depois.
                // Aqui é o "mínimo seguro" agora.
                if (SkillXP != null)
                    SkillXP.Clear();
            }
            catch
            {
                // Se alguma coisa não existir na sua versão, não vamos quebrar o uso da pedra.
            }

            // ===== 6) Reset do contexto da criação (virtual) =====
            if (OSUCreation == null)
                OSUCreation = new Server.Custom.Systems.Creation.Engine.OSUCreationContext();
            else
                OSUCreation.ResetAll();

            SendMessage(0x35, "Seu personagem foi resetado para iniciar a criação.");
        }

        #endregion

        #region OSU SISTEMA DE XP

        private Dictionary<SkillName, int> _SkillXP = new Dictionary<SkillName, int>();

        public Dictionary<SkillName, int> SkillXP
        {
            get
            {
                if (_SkillXP == null)
                    _SkillXP = new Dictionary<SkillName, int>();

                return _SkillXP;
            }
        }

        public int GetSkillXP(SkillName sk)
        {
            return _SkillXP != null && _SkillXP.TryGetValue(sk, out var v) ? v : 0;
        }

        public void AddSkillXP(SkillName sk, int amount)
        {
            /*  Skill s = Skills[sk];
              if (s == null || s.Lock != SkillLock.Up)
                  return;

              if (!IsOSUSkillUnlocked(sk))
                  return;
              ...
              _SkillXP.TryGetValue(sk, out var cur);
              _SkillXP[sk] = cur + amount;

              // Se quiser feedback:
               SendMessage(0x35, $"+{amount} XP em {sk}");
              */

            Server.SkillXp.SkillXPSystem.AwardXP(this, sk, amount);
        }
        ...
        public void AddGeneralXP(int amount)
        {
            ...
            // Feedback opcional:
            SendMessage(0x35, $"Você ganhou {amount} XP.");
        ...
        {
            if (OSULevel >= Server.SkillXp.SkillXPSystem.OSUMaxLevel)
                return;

            while (OSULevel < Server.SkillXp.SkillXPSystem.OSUMaxLevel && OSUGeneralXP >= OSUNextLevelXP)
            {
                OSULevel++;

                // Próximo nível (ajuste como quiser):
                // Exemplo simples: aumenta 1000 * nível
                OSUNextLevelXP += 1000 * OSULevel;

                // A cada nível, +1 escolha de habilidade travada (riding, línguas, etc.)
                OSUAbilityPicks++;

                SendMessage(0x35, string.Format(
                    "Você chegou ao nível {0}! Você ganhou 1 escolha de habilidade (total: {1}).",
                      OSULevel, OSUAbilityPicks));
            }
        }

        public void AddOSUAbility(int id)
        {
            if (_OSUAbilities == null)
                _OSUAbilities = new List<int>();

            if (!_OSUAbilities.Contains(id))
                _OSUAbilities.Add(id);
        }

        public bool IsOSUSkillUnlocked(SkillName skill)
        {
            if (_OSUUnlockedSkills == null)
                _OSUUnlockedSkills = new HashSet<SkillName>(); // default: tudo locked

            return _OSUUnlockedSkills.Contains(skill);
        }
        ...
        public void UnlockOSUSkill(SkillName skill)
        {
            ...
            _OSUUnlockedSkills.Add(skill);
        }

        public bool TryLockOSUSkill(SkillName skill)
        {
            if (_OSUUnlockedSkills == null)
                _OSUUnlockedSkills = new HashSet<SkillName>();

            _OSUUnlockedSkills.Remove(skill);

            // Se você quiser dar feedback aqui, pode, mas prefiro deixar o gump mandar a mensagem
            return true;
        }

        public void AddOSUFeat(int featId)
        {
            if (_OSUFeats == null)
                _OSUFeats = new List<int>();

            if (!_OSUFeats.Contains(featId))
                _OSUFeats.Add(featId);
        }

        public override void OnSkillsQuery(Mobile from)
        {
            // quando o jogador pede para ver as PRÓPRIAS skills (botão no paperdoll)
            if (from == this)
            {
                PlayerMobile pm = this;

                from.CloseGump(typeof(Server.Gumps.OSUSkillGump));
                from.SendGump(new Server.Gumps.OSUSkillGump(from, pm, 0, 0));
            }
            else
            {
                // GM vendo skills de outro mobile: mantém padrão
                base.OnSkillsQuery(from);
            }
        }

        public void OpenOSUSkillGump()
        {
            OSUSkillGumpOpened = true;

            CloseGump(typeof(Server.Gumps.OSUSkillGump));
            SendGump(new Server.Gumps.OSUSkillGump(this, this, 0, 0));
        }
        ...
        private TimeSpan GetOSUKnockoutDuration()
        {
            ...
            // Se foi um jogador que causou o desmaio, fica 3 minutos
            if (_OSUKnockoutFromPlayer)
                seconds = 180;
            else
                seconds = (int)Math.Round(OSUKnockoutSeconds * (OSUKnockoutScalar <= 0 ? 1.0 : OSUKnockoutScalar));

            if (seconds < 1)
                seconds = 1;

            seconds = OSUDefQualDispatcher.ModifyUnconsciousDurationSeconds(this, seconds);
        ...
        private TimeSpan GetOSULifeRegenInterval()
        {
            double seconds = OSULifeRegenSeconds * (OSULifeRegenScalar <= 0 ? 1.0 : OSULifeRegenScalar);
            if (seconds < 60)
                seconds = 60; // evita valores absurdos
            return TimeSpan.FromSeconds(seconds);
        }

        private void OSUEnsureLifeRegenTimer()
        {
            if (_OSULifeRegenTimer != null)
                return;

            _OSULifeRegenTimer = Timer.DelayCall(TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(1), OSULifeRegenTick);
        }

        private void OSULifeRegenTick()
        {
            if (Deleted)
            {
                _OSULifeRegenTimer?.Stop();
                _OSULifeRegenTimer = null;
                return;
            }

            OSUApplyLifeRegenCatchup();
        }

        private void OSUApplyLifeRegenCatchup()
        {
            if (OSULivesMax < 0)
                OSULivesMax = 0;

            if (OSULives > OSULivesMax)
                OSULives = OSULivesMax;

            if (OSULivesMax == 0)
            {
                _OSUNextLifeRegen = DateTime.MinValue;
                return;
            }

            // Se está cheio, não precisa de timer
            if (OSULives >= OSULivesMax)
            {
                _OSUNextLifeRegen = DateTime.MinValue;
                return;
            ...
            if (_OSUNextLifeRegen == DateTime.MinValue)
            {
                _OSUNextLifeRegen = DateTime.UtcNow + GetOSULifeRegenInterval();
                return;
            }

            var now = DateTime.UtcNow;

            if (now < _OSUNextLifeRegen)
                return;

            var interval = GetOSULifeRegenInterval();

            int gained = 1 + (int)((now - _OSUNextLifeRegen).TotalSeconds / interval.TotalSeconds);
            if (gained < 1)
                gained = 1;

            OSULives = Math.Min(OSULivesMax, OSULives + gained);

            if (OSULives >= OSULivesMax)
                _OSUNextLifeRegen = DateTime.MinValue;
            else
                _OSUNextLifeRegen = _OSUNextLifeRegen + TimeSpan.FromSeconds(interval.TotalSeconds * gained);
        }

        private void OSUConsumeLifeAndStartRegenIfNeeded()
        {
            if (OSULivesMax <= 0)
                return;

            if (OSULives < OSULivesMax && _OSUNextLifeRegen == DateTime.MinValue)
                _OSUNextLifeRegen = DateTime.UtcNow + GetOSULifeRegenInterval();

            OSUEnsureLifeRegenTimer();
        }
        ...
        private void OSUHandleKnockoutOnDeath(Container c)
        {
            ...
            if (c is Corpse corpse && corpse != null && !corpse.Deleted)
            {
                ...
                _OSUKnockoutCorpseSerial = corpse.Serial;
            }
            else
            {
                _OSUKnockoutCorpseSerial = 0;
            }

            // Move o fantasma para a área de desmaio
            try
            {
                MoveToWorld(OSUKnockoutLocation, OSUKnockoutMap);
            }
            catch
            {
                // se map inválido, ignora
            }

            var delay = GetOSUKnockoutDuration();

            Timer.DelayCall(delay, OSUWakeFromKnockout);

            SendMessage(0x35, $"Você desmaiou. Você vai acordar em {Math.Max(1, (int)delay.TotalSeconds)}s.");
        }


        private void OSUWakeFromKnockout()
        {
            if (Deleted)
                return;

            // Se por algum motivo já está vivo, não faz nada
            if (Alive)
                return;

            Corpse corpse = null;

            if (_OSUKnockoutCorpseSerial != 0)
                corpse = World.FindItem(_OSUKnockoutCorpseSerial) as Corpse;

            // Volta para o corpo ANTES de ressuscitar (efeito: fantasma "volta pro corpo")
            if (corpse != null && !corpse.Deleted)
            {
                try
                {
                    MoveToWorld(corpse.Location, corpse.Map);
                }
                catch
                {
                }
            ...
            // Remove DeathRobe, para permitir re-equip (se sobrou armadura/roupa no corpse)
            Item deathRobe = FindItemOnLayer(Layer.OuterTorso);
            ...
            // Se achou o corpse, tenta restaurar o que SOBROU nele
            if (corpse != null && !corpse.Deleted)
            {
                try
                {
                    OSURestoreRemainingItemsFromCorpse(corpse);

                    // Ao "acordar", o corpo não deve ficar no chão (só fica em permadeath)
                    if (!corpse.Deleted)
                        corpse.Delete();
                }
                catch
            ...
            // Animação simples de levantar (best-effort)
            try
            {
                if (Body.IsHuman && !Mounted)
                    Animate(21, 5, 1, true, false, 0);
            }
            catch
            {
            }
        ...
        private void OSURestoreRemainingItemsFromCorpse(Corpse corpse)
        {
            if (corpse == null || corpse.Deleted)
                return;
            ...
            // Copia lista porque vamos remover itens do container enquanto iteramos
            List<Item> items = new List<Item>(corpse.Items);

            // 1) tenta equipar primeiro (itens com layer válida)
            for (int i = 0; i < items.Count; i++)
            {
                ...
                // Se não está no corpse mais (alguém lootou), ignora
                if (item.Parent != corpse)
                    continue;

                // Itens "fixos" do corpse (ex: cabelo, etc) não mexemos
                if (item.Movable == false)
                    continue;

                // Só tenta equipar se for equipável e tiver layer válida
                if (item.Layer != Layer.Invalid &&
                    item.Layer != Layer.Bank &&
                    item.Layer != Layer.Backpack &&
                    item.Layer != Layer.Mount)
                {
                    // Remove do corpse e tenta equipar
                    corpse.RemoveItem(item);

                    // Se falhar equipar, joga na backpack
                    if (!TryEquipItem(item))
                        pack.DropItem(item);
                }
            }

            // 2) o resto vai pra backpack (incluindo o backpack antigo do corpse)
            items = new List<Item>(corpse.Items);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                // Se for o backpack antigo, traz o conteúdo pra mochila atual e descarta o container antigo
                if (item is Backpack oldPack)
                {
                    var oldItems = new List<Item>(oldPack.Items);
                ...
                // Qualquer outro item vai pra mochila
                pack.DropItem(item);
            }

            // Atualiza status/props (força update)
            Hits = Hits;
            Stam = Stam;
            Mana = Mana;

            SendMessage(0x35, "Você recuperou os itens que ainda estavam no seu corpo.");
        }
        ...
        public bool TryEquipItem(Item item)
        {
            ...
            try
            {
                ...
                // Se já existe item naquela layer, não força
                Item existing = FindItemOnLayer(item.Layer);
        ...
        private void OSUHandlePermaDeathOnDeath()
        {
            try
            {
                MoveToWorld(OSUHeavenLocation, OSUHeavenMap);
            }
            catch
            {
            }

            SendMessage(0x22, "Você morreu sem vidas restantes. Seu personagem está no céu. Fale com a staff se houve algum problema.");

            // Aqui você pode travar o player de várias formas; por enquanto só mantém como fantasma no céu.
        }
        ...
        private static void OnLogin(LoginEventArgs e)
        {
            ...
            {
                Custom.Systems.Needs.Gumps.OSUNeedsGump.TryRefresh(pm);
                //=============================================================
                //                          OSU OnLogin
                //=============================================================
                // Se for o primeiro login do personagem, trava TODAS as skills
                if (!pm.OSUInitialSkillsLocked)
                {
                ...
                // E garante que ninguém mantenha Down (Up/Locked apenas)
                for (int i = 0; i < pm.Skills.Length; i++)
                {
                    Skill sk = pm.Skills[i];
                    if (sk != null && sk.Lock == SkillLock.Down)
                    {
                        sk.SetLockNoRelay(SkillLock.Locked);
                        sk.Update();
                    }
                ...
                // OSU Lives: aplica recuperação offline/online e garante timer
                pm.OSUApplyLifeRegenCatchup();
                pm.OSUEnsureLifeRegenTimer();
        ...
        public override bool OnBeforeDeath()
        {
            ...
            #region OSU OnBeforeDeath

            // Killer mais recente (já calculado acima no seu código)
            PlayerMobile killerPM = m_InsuranceAward as PlayerMobile;

            // Foi jogador mesmo? (evita suicídio)
            bool killedByPlayer = (killerPM != null && killerPM != this);

            // Timer de desmaio: se foi jogador, 3 minutos (você já usa essa flag em GetOSUKnockoutDuration)
            _OSUKnockoutFromPlayer = killedByPlayer;

            // Flags PVP
            bool victimIsPvp = this.OSUIsPvpChar;
            bool killerIsPvp = (killerPM != null && killerPM.OSUIsPvpChar);

            // ===== Regra NOVA de consumir vida da VÍTIMA =====
            // - Se foi MOB: sempre consome vida (PVP e NoPVP)
            // - Se foi PLAYER:
            //    - se vítima é PVP: consome vida sempre
            //    - se vítima é NoPVP: consome vida só se o atacante é PVP
            bool consumeVictimLife = !killedByPlayer || victimIsPvp || killerIsPvp;

            // ===== Decide se é desmaio ou permadeath =====
            if (!_OSUKnockoutPending && !_OSUPermaDeathPending)
            {
                if (OSULivesMax > 0)
                {
                    if (consumeVictimLife)
                    {
                        if (OSULives > 0)
                        {
                            OSULives--;
                    ...
                    {
                        // Não consome vida, mas ainda desmaia normal
                        _OSUKnockoutPending = true;

                        if (killerPM != null)
                            killerPM.SendMessage(0x35, "Nenhuma vida foi perdida nesse desmaio.");
                    }
                }
            }

            // ===== Penalidade do agressor (NOVO) =====
            // Apenas quando:
            // - foi jogador,
            // - o atacante é PVP,
            // - a vítima é Não-PVP
            // => os dois perdem vida
            if (killedByPlayer && killerPM != null && killerIsPvp && !victimIsPvp)
            {
                if (killerPM.OSULives > 0)
                {
                    killerPM.OSULives--;
                    killerPM.SendMessage(0x35, "Você perdeu 1 vida por ter tirado uma vida de um personagem Não-PVP.");
                    killerPM.OSUConsumeLifeAndStartRegenIfNeeded();
                }
            }
        ...
        {
            // ===== OSU Lives handling =====
            // Se foi desmaio, move o fantasma para a área de KO e programa a volta.
            // Se foi permadeath, manda para o céu.
            if (_OSUKnockoutPending)
            {
                _OSUKnockoutPending = false;
                OSUHandleKnockoutOnDeath(c);
            }
            else if (_OSUPermaDeathPending)
            {
                _OSUPermaDeathPending = false;
                OSUHandlePermaDeathOnDeath();
            }
		...
		*/
        ...
        public override int MaxWeight
        {
            get
            {
                int baseWeight = (((Core.ML && Race == Race.Human) ? 100 : 40) + (int)(3.5 * Str));
                return OSUDefQualDispatcher.ModifyMaxWeight(this, baseWeight);
            }
        }
        ...
        public override int HitsMax
        {
            get
            {
                ...
                if (OSUCreationCompleted && OSUBaseHP > 0)
                    baseHits = OSUBaseHP; // HP escolhido no gump
                else
                    baseHits = (strBase / 2) + 50; // comportamento padrão

                return baseHits + strOffs;
        ...
        public override int StamMax
        {
            get
            {
                int baseStam = (OSUCreationCompleted && OSUBaseStam > 0) ? OSUBaseStam : base.StamMax;
                return baseStam + AosAttributes.GetValue(this, AosAttribute.BonusStam);
            }
        }
        ...
        public override int ManaMax
        {
            get
            {
                int baseMana = (OSUCreationCompleted && OSUBaseMana > 0) ? OSUBaseMana : base.ManaMax;
        ...
        public override int Str
        {
            get
            {
                ...
                {
                    int str = base.Str;
                    return Math.Min(str, Math.Min(StrMaxCap, OSUStrCapMax));
                }
        ...
        public override int Int
        {
            get
            {
                ...
                {
                    int intel = base.Int;
                    return Math.Min(intel, Math.Min(IntMaxCap, OSUIntCapMax));
                }
        ...
        public override int Dex
        {
            get
            {
                ...
                {
                    int dex = base.Dex;
                    return Math.Min(dex, Math.Min(DexMaxCap, OSUDexCapMax));
                }
        ...
        public PlayerMobile()
        {
            ...
            #region OSU Constructors

            // ===== SISTEMA DE XP CUSTOM =====
            OSUGeneralXP = 0;
            OSULevel = 0;
            OSUNextLevelXP = 1000;
            OSUAbilityPicks = 0;
            ...
            OSUFeatSpentXP = 0;
            OSUFeatSpentXPCombat = 0;
            OSUFeatSpentXPProf = 0;
            OSUFeatCapsInverted = false;
            OSUInitialSkillsLocked = false;

            // OSU Lives defaults
            OSULivesMax = 3;
            OSULives = 3;
            OSULifeRegenSeconds = 3600;
            OSUKnockoutSeconds = 60;
            OSULifeRegenScalar = 1.0;
            OSUKnockoutScalar = 1.0;
            _OSUNextLifeRegen = DateTime.MinValue;
            _OSUKnockoutCorpseSerial = 0;
            _OSUKnockoutPending = false;
            _OSUPermaDeathPending = false;
            ...
            // Se você quiser que novos personagens comecem com skills específicas liberadas,
            // você vai preencher isso no sistema de criação do char (mais tarde).
            // Aqui eu deixo null para manter backward compat (tudo liberado até você setar).
            _OSUUnlockedSkills = new HashSet<SkillName>(); // começa com tudo locked
        ...
        public override ApplyPoisonResult ApplyPoison(Mobile from, Poison poison)
        {
            ...
            double poisonScalar = OSUDefQualDispatcher.ModifyPoisonSusceptibility(this, 1.0);

            if (poisonScalar >= 1.10 && Utility.RandomDouble() < (poisonScalar - 1.0))
                poison = PoisonImpl.IncreaseLevel(poison);
            else if (poisonScalar <= 0.90 && Utility.RandomDouble() < (1.0 - poisonScalar))
                poison = PoisonImpl.DecreaseLevel(poison);
        ...
        {
            if (OSUAvatarId > 0)
                Server.Custom.Systems.Creation.Engine.OSUAvatarRegistry.UnmarkUsed(OSUAvatarId);
        ...
        public override int ComputeMovementSpeed(Direction dir, bool checkTurning)
        {
            ...
            int speed = (running ? RunFoot : WalkFoot);

            speed = OSUDefQualDispatcher.ModifyRunSpeed(this, speed, running);
        ...
        {
            if (e.Speech != null && e.Speech.StartsWith("."))
            {
                // bloqueia aparecer sobre a cabeça
                e.Blocked = true;
                return;
            }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                ...
                case 50:
                    {
                        // fome e sede
                        OSUNeedsIntPenalty = reader.ReadInt();
                        OSUNeedsStrPenalty = reader.ReadInt();
                        OSUNeedsDexPenalty = reader.ReadInt();
                        OSUThirst = reader.ReadInt();
                        OSUHunger = reader.ReadInt();
                        goto case 49;
                    }
                case 49:
                    {
                        // Creation 
                        OSUCombatSkillCap = reader.ReadDouble();
                        OSUCraftSkillCap = reader.ReadDouble();
                        _OSURpAgeBase = reader.ReadInt();
                        OSURpBirthWorldTime = reader.ReadDateTime();
                        OSU_EvalCarry = reader.ReadInt();
                        OSU_TacticsCarry = reader.ReadInt();
                        OSUCitizenCityId = reader.ReadString();
                        OSUPendingStatPoints = reader.ReadInt();
                        OSUCreationCompleted = reader.ReadBool();
                        OSUIsPvpChar = reader.ReadBool();
                        OSUCultureId = reader.ReadString();
                        OSUReligionId = reader.ReadString();
                        OSUAvatarId = reader.ReadInt();
                        OSURpWeightKg = reader.ReadInt();
                        OSURpHeightCm = reader.ReadInt();
                        OSURpAge = reader.ReadInt();
                        OSURpTraitsPublic = reader.ReadString();
                        OSURpHistoryStaff = reader.ReadString();
                        OSUFeatTotalCapCustom = reader.ReadInt();
                        OSUFeatCombatCapCustom = reader.ReadInt();
                        OSUFeatProfessionCapCustom = reader.ReadInt();
                        goto case 48;
                    }
                case 48:
                    {
                        bool hasCtx = reader.ReadBool();
                        OSUCreation = hasCtx ? OSUCreationContext.Deserialize(reader) : new OSUCreationContext();
                        goto case 47;
                    }
                case 47:
                    {
                        OSUPermaDead = reader.ReadBool();
                        goto case 46;
                    }
                case 46:
                    {
                        OSUInitialSkillsLocked = reader.ReadBool();

                        // OSU Lives
                        OSULivesMax = reader.ReadInt();
                        OSULives = reader.ReadInt();
                        OSULifeRegenSeconds = reader.ReadInt();
                        OSUKnockoutSeconds = reader.ReadInt();
                        OSULifeRegenScalar = reader.ReadDouble();
                        OSUKnockoutScalar = reader.ReadDouble();
                        _OSUNextLifeRegen = reader.ReadDateTime();
                ...
                case 45:
                    {
                        ...
                        // OSU Lives defaults (versão antiga)
                        if (OSULivesMax <= 0)
                            OSULivesMax = 3;
                        ...
                        if (OSULifeRegenSeconds <= 0)
                            OSULifeRegenSeconds = 3600;

                        if (OSUKnockoutSeconds <= 0)
                            OSUKnockoutSeconds = 60;

                        if (OSULifeRegenScalar <= 0)
                            OSULifeRegenScalar = 1.0;

                        if (OSUKnockoutScalar <= 0)
                            OSUKnockoutScalar = 1.0;
                ...
                case 44: // OSU abilities + OSU caps/locks
                    {
                        int abCount = reader.ReadInt();
                        _OSUAbilities = new List<int>(abCount);
                        for (int i = 0; i < abCount; i++)
                            _OSUAbilities.Add(reader.ReadInt());

                        goto case 43;
                    }

                case 43: // OSU XP System (novo)
                    {
                        OSUFeatSpentXP = reader.ReadInt();
                        OSUFeatSpentXPCombat = reader.ReadInt();
                        OSUFeatSpentXPProf = reader.ReadInt();
                        OSUFeatCapsInverted = reader.ReadBool();

                        int featCount = reader.ReadInt();
                        _OSUFeats = new List<int>(featCount);
                        for (int i = 0; i < featCount; i++)
                            _OSUFeats.Add(reader.ReadInt());

                        OSUGeneralXP = reader.ReadInt();
                        OSULevel = reader.ReadInt();
                        OSUNextLevelXP = reader.ReadInt();
                        OSUAbilityPicks = reader.ReadInt();

                        int count = reader.ReadInt();
                        _SkillXP = new Dictionary<SkillName, int>(count);
                        ...
                        int unlockedCount = reader.ReadInt();
                        _OSUUnlockedSkills = new HashSet<SkillName>();
                        for (int i = 0; i < unlockedCount; i++)
                            _OSUUnlockedSkills.Add((SkillName)reader.ReadInt());

                        // IMPORTANTE: aqui termina. Não pode cair em case 42.
                        goto case 41; // ou break; dependendo do seu switch principal
                    }

                case 42: // OSU XP System (antigo)
                    {
                        OSUFeatSpentXP = reader.ReadInt();
                        ...
                        // Agora você quer todos começando locked:
                        _OSUUnlockedSkills = new HashSet<SkillName>();
                ...
                case 16:
                    {
                        ...
                        int count2 = reader.ReadEncodedInt();
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(OSUNeedsStrPenalty);
            writer.Write(OSUNeedsDexPenalty);
            writer.Write(OSUThirst);
            writer.Write(OSUHunger);

            //Creation
            writer.Write(OSUCombatSkillCap);
            writer.Write(OSUCraftSkillCap);
            writer.Write(_OSURpAgeBase);
            writer.Write(OSURpBirthWorldTime);
            writer.Write(OSU_EvalCarry);
            writer.Write(OSU_TacticsCarry);
            writer.Write(OSUCitizenCityId);
            writer.Write(OSUPendingStatPoints);
            writer.Write(OSUCreationCompleted);
            writer.Write(OSUIsPvpChar);
            writer.Write(OSUCultureId);
            writer.Write(OSUReligionId);
            writer.Write(OSUAvatarId);
            writer.Write(OSURpWeightKg);
            writer.Write(OSURpHeightCm);
            writer.Write(OSURpAge);
            writer.Write(OSURpTraitsPublic);
            writer.Write(OSURpHistoryStaff);
            writer.Write(OSUFeatTotalCapCustom);
            writer.Write(OSUFeatCombatCapCustom);
            writer.Write(OSUFeatProfessionCapCustom);

            writer.Write(OSUCreation != null);
            if (OSUCreation != null)
            {
                OSUCreation.Serialize(writer); // ou OSUCreationContext.Serialize(writer)
            }

            // ====== (case 47) PERMADEAD ======
            writer.Write(OSUPermaDead);

            // ====== (case 46) LOCK SKILLS + LIVES ======
            writer.Write(OSUInitialSkillsLocked);

            // OSU Lives
            writer.Write(OSULivesMax);
            writer.Write(OSULives);
            writer.Write(OSULifeRegenSeconds);
            writer.Write(OSUKnockoutSeconds);
            writer.Write(OSULifeRegenScalar);
            writer.Write(OSUKnockoutScalar);
            writer.Write(_OSUNextLifeRegen);

            // OSU Abilities
            writer.Write(_OSUAbilities != null ? _OSUAbilities.Count : 0);
            if (_OSUAbilities != null)
            {
                for (int i = 0; i < _OSUAbilities.Count; i++)
                    writer.Write(_OSUAbilities[i]);
            }

            // OSU Feats (caps)
            writer.Write(OSUFeatSpentXP);
            writer.Write(OSUFeatSpentXPCombat);
            writer.Write(OSUFeatSpentXPProf);
            writer.Write(OSUFeatCapsInverted);

            // OSU Feats list
            writer.Write(_OSUFeats != null ? _OSUFeats.Count : 0);
            if (_OSUFeats != null)
            {
                for (int i = 0; i < _OSUFeats.Count; i++)
                    writer.Write(_OSUFeats[i]);
            }

            // OSU XP Geral
            writer.Write(OSUGeneralXP);
            writer.Write(OSULevel);
            writer.Write(OSUNextLevelXP);
            writer.Write(OSUAbilityPicks);

            // SkillXP dict
            writer.Write(_SkillXP != null ? _SkillXP.Count : 0);
            if (_SkillXP != null)
            {
            ...
            // Unlocked skills
            writer.Write(_OSUUnlockedSkills != null ? _OSUUnlockedSkills.Count : 0);
            if (_OSUUnlockedSkills != null)
            {
                foreach (var sk in _OSUUnlockedSkills)
                    writer.Write((int)sk);
            }
