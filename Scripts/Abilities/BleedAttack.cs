...
namespace Server.Items
{
    ...
    public class BleedAttack : WeaponAbility
    {
		...
		public static void BeginBleed(Mobile m, Mobile from, bool splintering = false)
        {
            ...
            if (pm != null)
            {
                double bleedScalar = OSUDefQualDispatcher.ModifyBleedSusceptibility(pm, 1.0);

                if (bleedScalar <= 0.90 && Utility.RandomDouble() < (1.0 - bleedScalar))
                    return;
            }
        ...
        public static void DoBleed(Mobile m, Mobile from, int damage, bool blooddrinker)
        {
            if (m.Alive && !m.IsDeadBondedPet)
            {
                ...
                PlayerMobile pm = m as PlayerMobile;
                if (pm != null)
                    damage = OSUDefQualDispatcher.ModifyBleedTickDamage(pm, damage);
