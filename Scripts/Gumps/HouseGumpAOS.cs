                ...
                case HouseGumpPageAOS.Storage:
                    {
                        ...
                        {
                            fromSecures = house.SecureCount;
                            fromVendors = 0;
                            fromMovingCrate = 0;

                            // lockdowns reais do jeito que você quer:
                            // itens travados normais + quantidade de secure containers
                            fromLockdowns = house.GetLockdowns() + house.SecureCount;

                            maxSecures = house.MaxSecures;
                            curSecures = house.SecureCount;

                            maxLockdowns = house.MaxLockDowns;
                            curLockdowns = house.GetLockdowns() + house.SecureCount;
                        }
                        else
                        {
                            maxSecures = house.GetAosMaxSecures();
                            curSecures = house.GetAosCurSecures(out fromSecures, out fromVendors, out fromLockdowns, out fromMovingCrate);

                            maxLockdowns = house.GetAosMaxLockdowns();
                            curLockdowns = house.GetAosCurLockdowns();
                        }
