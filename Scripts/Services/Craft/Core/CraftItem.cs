...
using Server.Custom.Systems.Crafting.Cooking.Consumables.Foods;
#endregion
			...
			CustomCraft customCraft)
		{
			...
			if (CheckSkills(from, typeRes, craftSystem, ref ignored, ref allRequiredSkills, checkMaxAmount))
			{
				...
				if (num == 0)
				{
                    ...
                    try
                    {
                        // Só dá XP se o craft NÃO falhou
                        if (allRequiredSkills)
                        {
                            ...
                            if (pm != null)
                            {
                                ...
                                // Skill principal do craft (Blacksmith, Tailoring, etc)
                                SkillName craftSkill = craftSystem.MainSkill;

                                // ===== MIN SKILL DA RECEITA =====
                                // Usa a lista Skills do CraftItem (igual você fez)
                                double minSkill = 0.0;

                                for (int i = 0; i < Skills.Count; i++)
                                {
                                    CraftSkill cs = Skills.GetAt(i);

                                    if (cs != null && cs.SkillToMake == craftSystem.MainSkill)
                                    {
                                        minSkill = cs.MinSkill - MinSkillOffset;
                                        break;
                                    }
                                }
                                ...
                                // ===== XP BASE = 10% DO MIN SKILL =====
                                int baseXP = Server.SkillXp.SkillXPSystem.CalculateCraftBaseXP(minSkill);

                                // ===== BÔNUS EXCEPTIONAL =====
                                int exceptionalBonus = 0;
                                if (quality == 2) // exceptional
                                {
                                    exceptionalBonus = (int)Math.Ceiling(
                                        baseXP * Server.SkillXp.SkillXPSystem.OSUCraftExceptionalBonusPercent
                                    );
                                }
                                ...
                                if (typeRes != null)
                                    cr = CraftResources.GetFromType(typeRes);

                                if (cr != CraftResource.None)
                                    materialBonus = Server.SkillXp.SkillXPSystem.GetCraftMaterialBonus(cr);

                                // ===== BÔNUS POR RECEITA ESPECÍFICA =====
                                int recipeBonus = Server.SkillXp.SkillXPSystem.GetCraftRecipeXPBonus(this.ItemType);

                                // ===== TOTAL =====
                                int totalXP = baseXP + exceptionalBonus + materialBonus + recipeBonus;

                                if (totalXP < 1)
                                    totalXP = 1;

                                // ===== DAR XP =====
                                Server.SkillXp.SkillXPSystem.AwardXP(pm, craftSkill, totalXP);
                    ...
                    {
                        // Se quiser ver erro no console:
                        Console.WriteLine("OSU CRAFT XP ERROR: " + e);
                    }
                    #endregion
