...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        public bool LockDown(Mobile m, Item item, bool checkIsInside)
        {
            ...
            {
                TownHouse rentHouse = this as TownHouse;
                OSUPropertyType rentType = OSUPropertyType.House;

                if (rentHouse != null && rentHouse.ForSaleSign != null)
                    rentType = rentHouse.ForSaleSign.PropertyType;

                bool isRentCommercialOrTomb = rentHouse != null && rentType != OSUPropertyType.House;

                if (isRentCommercialOrTomb && item is Container)
                {
                    m.SendMessage("Nesta propriedade você não pode dar lockdown em containers.");
                    return false;
                }
                ...
                else if (!(item is VendorRentalContract) && (IsAosRules ? !CheckAosLockdowns(amt) : (LockDownCount + amt) > MaxLockDowns))
                {
        ...
        public void AddSecure(Mobile m, Item item)
        {
            ...
            else
            {
                ...
                else if (rentHouse != null && rentType != OSUPropertyType.House)
                {
                    m.SendMessage("Esta propriedade não permite secure containers.");
                }
                else if (rentHouse != null && Secures.Count >= MaxSecures)
                {
                    m.SendMessage("Você já atingiu o limite máximo de secures desta propriedade.");
                }
                else if (rentHouse == null && !IsAosRules && SecureCount >= MaxSecures)
                {
